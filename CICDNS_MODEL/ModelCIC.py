...
class DomainGraph(nn.Module):
    def __init__(self, n_node, args: dict, class_weights, n_categories):
        ...
        self.embedding = nn.Embedding(self.n_node + 1, self.initial_feature, padding_idx=0)  # Maps nodes into vector space of the initial feature size
        ...
        self.conv1_3 = nn.Conv1d(self.initial_feature, self.initial_feature * 2, 3, padding=1)  # Used to extract character features
        ...
        self.ln1 = nn.Linear(Data.max_domainname_len * 3, Data.max_domainname_len)  # It is used for dimensionality reduction
        ...
        self.lstm1 = nn.LSTM(input_size=self.initial_feature, hidden_size=self.hidden_size)
    ...
    def forward(self, X, G, last_feas=None):
        # print(X.shape)
        node=list()
        hidden = self.embedding(X)  # Encode domain name strings into vector numbers
        # print(hidden)
        # Do some convolution before the character special diagnosis, otherwise the effect will be poor
        # print(hidden.shape)
        # print("Before permute:", hidden.shape)
        hidden = hidden.permute(0, 2, 1)

        # print("---------------",hidden)
        # print("After permute:", hidden.shape)
        hidden1_3 = self.relu(self.conv1_3(hidden))
        # print("-----3----------", hidden1_3.shape )
        hidden1_5 = self.relu(self.conv1_5(hidden))
        # print("------5---------", hidden1_5.shape)
        hidden1_7 = self.relu(self.conv1_7(hidden))  # Extract semantic features from different character fragments
        # print("--------7-------", hidden1_7.shape)
        ...
        if last_feas is not None and len(last_feas[0]) != len(hidden[0]):  # Perform feature fusion
            hidden = hidden + last_feas.data
        # Enter the SuperGraph Training
        nodes = self.network(hidden, G)
        node.append(nodes)
        if len(node) >= 2:
            # LSTM fusion of the newly obtained nodes and the penultimate element
            nodes, _ = self.lstm1(node[-2:], node[-1])
        # Gather node features into the graph and reduce dimensionality
...
if __name__ == '__main__':
    ...
    # *******************************Subject***********************************
    data_rate = 1
    data = read_file("./data/test.json")
    domainnames = [item.Domain for item in data]
    ...
    labels_all = list()
    for item in data:
        if item.type not in labels_idx:
            labels_idx[item.type] = len(labels_idx)
        labels_all.append(item.type)
    ...
    for epoch in range(1):
        ...
        test_split = 12
        slices_test = test_data.generate_batch(params_global.get("batchSize"))
        ...
        pre = precision_score(test_labels, test_pred, zero_division=1,average='weighted')
        rec = recall_score(test_labels, test_pred,zero_division=1, average='weighted')
        f1 = f1_score(test_labels, test_pred, zero_division=1,average='weighted')
        print("acc:", acc)
        print("pre:", pre)
        print("rec:", rec)
        print("f1:", f1)
